...
void handleSpecialControlOption(char key, Route* route, GamePageData* data)
{
    switch (key)
    {
        ...
        case 'S':
        {
            char path[50];
            int validInputCount;
            data->tmpData->shouldUpdateTuiInformation = false;
            ...
            if (!exportGameRecordTableToFile(data->recordTable, path)) {
                printWithDoubleBuffer(
                    ...
                    printf("Tip: Target file will be created if not exists. Try to check related permissions.\n");
            ...
            printWithDoubleBuffer(
                printf("Target " GREEN_TEXT("%s") " saved successfully. Press enter to continue. "
        ...
        default:
        {
            abortWithMsg("Invalid special option.");
            return;
        }
    }
}
...
#define playWinnerSound(WINNER, MACHINE_SIDE) do { \
    if ((WINNER) == (MACHINE_SIDE)) \
        playMachineWinSound(); \
    else \
        playPlayerWinSound(); \
} while (0)
...
Route* updateGamePage(PageData* data, Route* routePassByGame)
{
    ...
    if (!isEmptySide(gameData->tmpData->winner)) {
        playWinnerSound(gameData->tmpData->winner, gameData->prefabConfig->machineSide);
        printWithDoubleBuffer(printGameOverHint(gameData->tmpData->winner));
        clearInputBuffer();
...
get_move_from_user:
    ...
    if (gameData->tmpData->isLastMoveConfirmed) playMoveSound();
...
{
    if (!data->tmpData->shouldUpdateTuiInformation) {
        data->tmpData->shouldUpdateTuiInformation = true;
        return;
    }
